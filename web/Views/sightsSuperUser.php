...
<body class="bg-gray-50 relative">
    ...
    <div class="container mx-auto pt-10">
        <?php foreach ($sights as $sight): ?>
            <div id="<?= $sight['pon_nome'] ?>" class="sight p-4 text-lg text-left w-full border rounded-md border-transparent hover:scale-105 transform ease-in-out hover:bg-gray-200 flex items-center" data-sight="<?= $sight['pon_nome'] ?>" data-accessibility="<?= $sight['pon_acessibilidade'] ?>" data-classification="<?= $sight['pon_classificacao'] ?>" data-drescripition="<?= $sight['pon_descricao'] ?>" data-stret="<?= $sight['pon_rua'] ?>" data-neighborhood="<?= $sight['pon_bairro'] ?>" data-city="<?= $sight['pon_cidade'] ?>">
                <div onclick="showDiv(formUpdateSight)">
                    <h2 class="font-semibold"><?= $sight['pon_nome'] ?></h2>
                    <p><?= substr($sight['pon_descricao'], 0, 400) . '...' ?></p>
                </div>
                <div class="space-y-5">
